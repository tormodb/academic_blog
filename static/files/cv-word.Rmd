...
"2017-",   "5", "PhD", "Faculty of Psychology, University of Bergen, Norway",
...
n_firstauthor <- res %>% select(V1) %>% filter(grepl('Bøe', V1)) %>% tally()
n_lastauthor <- tmp.last %>% filter(grepl('Bøe', V2)) %>% tally()
...
```{r, results='asis'}
bib<- ReadBib("tpubs_zot.bib",check= FALSE)
